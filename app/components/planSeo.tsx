...
export default function PlanPageSEO({ tripId, data }:{ tripId: string, data: any }) {
  const [tripDetails, setTripDetails] = useState(data.metadata);
  const [dataJSON, setDataJSON] = useState(data.tripplan);
    ...
    fetch('https://ipinfo.io/json')
      ...
      .then((locationData) => {
        ...
        console.log("Currency Code:", currencyCode);
            costDetailsRef.current = costProcessor(dataJSON);
            setTotalCost(costDetailsRef.current.totalcost);
            updateGraphicalCostData(costDetailsRef.current);

            placesNames.current.push(...Object.values(dataJSON.trip.trip).flatMap((day) => (day as Day).places.map(place => `${place.name}, ${ params.get('destination') || ''}`)));
...
{!["export", "bag"].includes(sideSelected) && (
      <DraggerComponent
        leftWidth={leftWidth}
        setLeftWidth={(n:number)=>{
          setLeftWidth(n);
        }}
        drag_direction={drag_direction}
        setDragDirection={(dir:string | null)=>{
